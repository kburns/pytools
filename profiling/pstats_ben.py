...
import math
# replace with statistics after March 2014 or so
import numpy as np
            ...
            for func, stat in item.stats.items():
                if func in self.stats:
                    ...
                    if not isinstance(cc, list):
                        ...
                        tt_list = [tt]
                        ct_list = [ct]
                    old_func_stat = (cc_list, nc_list, tt_list, ct_list, callers)
                    print(old_func_stat)
                else:
                    old_func_stat = ([], [], [], [], {},)
                self.stats[func] = append_func_stats(old_func_stat, stat)
        return self
    
    
    def average(self, *arg_list):
        if not arg_list:
            return self
        
        N_items = len(arg_list)
        self.add(*arg_list)

        self.total_calls /= N_items
        self.prim_calls /= N_items
        self.total_tt /= N_items
        
        for func, stat in self.stats.items():
            # we're not normalizing both parts (the dictionary part is getting left behind)
            self.stats[func] = normalize_func_stats(stat, N_items)
        return self

    def stddev(self, *arg_list):
        # do running stddev, following Knuth as reported in http://www.johndcook.com/standard_deviation.html
        if not arg_list:
            return self
        self.append(*arg_list)

        for func, stat_list in self.stats.items():
            self.stats[func] = stddev_collection_func_stats(stat_list)

        return self

    def mean(self, *arg_list):
        # do running stddev, following Knuth as reported in http://www.johndcook.com/standard_deviation.html
        if not arg_list:
            return self
        ...
        for func, stat_list in self.stats.items():
            print(self.stats[func])
            self.stats[func] = mean_collection_func_stats(stat_list)
...
def normalize_func_stats(stat, norm):
    """Normalize profile entries."""
    cc, nc, tt, ct, callers = stat
    return (int(cc/norm), int(nc/norm), tt/norm, ct/norm, callers)

def abs_diff_stats(stat, average):
    """Take the absolute value of the difference for all the stats for two profile entries."""
    cc, nc, tt, ct, callers = stat
    a_cc, a_nc, a_tt, a_ct, a_callers = average
    return (math.fabs(cc-a_cc), math.fabs(nc-a_nc), math.fabs(tt-a_tt), math.fabs(ct-a_ct),
              callers)

def append_func_stats(target, source):
    """Add together all the stats for two profile entries."""
    cc, nc, tt, ct, callers = source
    t_cc, t_nc, t_tt, t_ct, t_callers = target
    print("in append:", t_cc, cc, t_cc.append(cc), " ; ", t_tt, tt, t_tt.append(tt))
    
    return (t_cc.append(cc), t_nc.append(nc), t_tt.append(tt), t_ct.append(ct),
              add_callers(t_callers, callers))

def stddev_collection_func_stats(source):
    """Calculate stddev for a list of stats."""
    cc, nc, tt, ct, callers = source
    
    return (np.std(cc), np.std(nc), np.std(tt), np.std(ct), callers)

def mean_collection_func_stats(source):
    """Average list of stats."""
    cc, nc, tt, ct, callers = source
    
    return (np.mean(cc), np.mean(nc), np.mean(tt), np.mean(ct), callers)
